...
router.get('/books',async function(req,res,next){
    ...
    var s=parseInt(sid);
    var query="SELECT b1.BOOK_ID,b1.BOOK_NAME,b1.PRICE_PER_BOOK FROM BOOK b1,(SELECT DISTINCT BOOK_ID FROM BOOK_COPY WHERE SHOP_ID="+s+") b2 WHERE b1.BOOK_ID=b2.BOOK_ID";
    var params=[];
    let result=await logger.log(query,params);
    console.log(result);
...
router.post('/selectedbook',async function(req,res,next){
    ...
    console.log(sid);
    console.log(bid);
    var chq="SELECT * from cart_details WHERE CUSTOMER_ID="+uid+" and BOOK_ID="+bid;
    var params=[];
    let ck_result=await logger.log(chq,params);
    if(ck_result[0]==null){
        var query="INSERT INTO CART_DETAILS VALUES("+bid+"," +uid+","+sid+","+pr+")";
...
router.post('/createorder',async(req,res,next)=>{
    ...
    var t=pr*q;
    var params=[];
    var qr="SELECT COUNT(*) QUANTITY FROM BOOK_COPY WHERE SHOP_ID="+sid+" AND BOOK_ID="+bid;
    ...
    if(q>chq){
        ...
        var qery2="DECLARE BEGIN ADD_ORDER("+bid+", "+uid+","+sid+","+q+","+t+", '"+info.trn_id+"', '"+info.street+"', '"+info.thana+"',"+info.pcode+", '"+info.dist+"'); END;";
...
router.post('/clearcart',async(req,res,next)=>{
    ...
    var params=[];
    var qr="DELETE from cart_details WHERE CUSTOMER_ID="+uid+" and BOOK_ID="+bid;
    let result=await logger.log(qr,params);
    res.render('custpro',{d:info.uid});
...
router.get('/cartinfo',async(req,res,next)=>{
    ...
    console.log(id);
    var params=[];
    var qery2="SELECT b1.BOOK_NAME,b1.PRICE_PER_BOOK,b2.BOOK_ID,b2.shop_id,b2.CUSTOMER_ID,s.SHOP_NAME FROM BOOK b1 join (SELECT  BOOK_ID,shop_id,customer_id FROM Cart_details WHERE customer_ID="+id+") b2 on( b1.BOOK_ID=b2.BOOK_ID) join (SELECT SHOP_ID,SHOP_NAME from SHOP) s on( b2.SHOP_ID=s.SHOP_ID )";
    let re=await logger.log(qery2,params);
    res.render('cart',{d:id,cd:re})
})
router.get('/home2',async(req,res,next)=>{
    var id=req.query.id;
    var q="SELECT CUSTOMER_ID,FIRST_NAME FROM CUSTOMER WHERE CUSTOMER_ID ="+id;
    var params=[];
...
router.get('/customerinfo',async(req,res,next)=>{
    ...
    var tmp_q="SELECT (FIRST_NAME||' '||LAST_NAME) NAME FROM CUSTOMER WHERE CUSTOMER_ID="+cid;
    var params=[];      
...
router.post('/searchbyname', async function(req,res,next){
    ...
    var info=req.body;
    console.log(info);
    var q="select book_name,book_id,price_per_book from book where upper(book_name)=upper('"+info.bookname+"')";
    var pa=[];
...
router.get('/searchresult',async(req,res,next)=>{
    ...
    var tmp_q="SELECT s.SHOP_NAME,s.SHOP_ID from SHOP s join (SELECT DISTINCT SHOP_ID FROM BOOK_COPY WHERE BOOK_ID=(SELECT BOOK_ID from BOOK WHERE UPPER(BOOK_NAME)=UPPER('"+ bname+"'))) s2 ON(s.SHOP_ID = s2.SHOP_ID)";
    var params=[];      
    let result=await logger.log(tmp_q,params);
    console.log(result);
    res.status(200).json(result);
...
router.get('/bookcat',async function(req,res,next){
    //console.log(__dirname)
    var info=req.query;
    var q="SELECT BOOK_NAME,PRICE_PER_BOOK,BOOK_GENRE FROM BOOK WHERE UPPER(BOOK_GENRE) LIKE '%"+ info.gn+"%'";
    var pa=[];
    let result=await logger.log(q,pa);
...
router.get('/shoporder',async(req,res,next)=>{
    var id=req.query.id;
    var tmp_q="SELECT o.CUSTOMER_ID,o.ADDRESS_ID,b.BOOK_NAME,o.TRANSACTION_ID,o.QUANTITY,o.ORDER_ID,o.SHOP_ID from (SELECT BOOK_ID, QUANTITY,ORDER_ID,SHOP_ID, CUSTOMER_ID, ADDRESS_ID, TRANSACTION_ID FROM ORDERS WHERE SHOP_ID="+id+") o join BOOK b ON(o.BOOK_ID=b.BOOK_ID)";
    var params=[];      
    let result=await logger.log(tmp_q,params);
    console.log(result);
...
router.get('/showbooksall',async(req,res,next)=>{
    ...
    var q="SELECT (SELECT BOOK_NAME FROM BOOK B1 WHERE B1.BOOK_ID= B2.BOOK_ID) BOOK_NAME, COUNT(*) QUANTITY FROM BOOK_COPY B2 WHERE SHOP_ID="+sid+" GROUP BY BOOK_ID";
    var p=[];
...
router.get('/bookdetail',async function(req,res,next){
    ...
    //console.log(id);
    var query="SELECT AUTHOR_NAME,BOOK_NAME,BOOK_GENRE,SUMMARY,PUBLISHER_NAME FROM AUTHOR a join (SELECT BOOK_NAME,BOOK_GENRE,SUMMARY,AUTHOR_ID,PUBLISHER_ID from BOOK where BOOK_ID="+id+") b ON(a.AUTHOR_ID = b.AUTHOR_ID) join PUBLISHER p on(b.PUBLISHER_ID= p.PUBLISHER_ID)";
    var params=[];
    let result=await logger.log(query,params);
    //console.log(result);
...
router.post('/stempregister',async(req,res,next)=>{
    ...
    if(pass!=info.password[1]){
        ...
        if(result1[0]==null){
            console.log('insert');
            var q="INSERT into Temp_Shop (shop_name,email,street,thana,pcode,dist,username,pass,contact_no,description,trn_id) VALUES ('"+info.name +"','"+info.mail +"','"+info.street+"','"+info.thana+"','"+info.pcode +"','"+info.dist+"','"+info.username+"','"+pass+"','"+info.contact+"','"+info.des+"','"+info.trn_id+"')";
...
router.post('/loginId',async(req,res,next)=>{
    ...
    if(role=='customer'){
        ...
        query="SELECT ADMIN_ID,NAME,USERNAME FROM ADMIN WHERE PASS='"+user.pass+"'";
        console.log(user.pass);
    }
    else{
...
router.post('/bookadd',async (req,res,next)=>{
    ...
    console.log('insert');
    var params=[];
    var q="DECLARE BEGIN add_book("+sid+", '"+info.name +"','"+info.genre+"','"+info.author+"','"+info.publisher+"','"+info.lan+"','"+info.des+"','"+info.isbn+"',"+pages+","+qn+",'"+info.edition+"',"+price+"); END;";
    let result=await logger.log(q,params);
...
router.get('/address',async(req,res,next)=>{
    ...
    var query="SELECT a.STREET, a.THANA, a.POSTAL_CODE, d.DISTRICT_NAME from ADDRESS_DETAIL a join DISTRICT d on(a.DISTRICT_ID = d.DISTRICT_ID) WHERE a.ADDRESS_ID="+q;
    var params=[];
    let result=await logger.log(query,params);
    //console.log(result);
    res.status(200).json(result);
...
router.post('/publisheradd',async(req,res,next)=>{
    ...
    var info=req.body;
    console.log(info);
    var q="DECLARE BEGIN add_publisher('"+ info.name+"', '"+info.summ+"', '"+info.street+"', '"+info.thana+"',"+info.pcode+", '"+info.dist+"'); END;";
